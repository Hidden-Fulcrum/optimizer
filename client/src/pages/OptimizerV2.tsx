...
import { ArrowLeft, Calculator, Clock, Save, Sparkles, AlertCircle } from "lucide-react";
...
export default function OptimizerV2() {
  ...
  const getTaskColor = (category: string): string => {
    return category === 'critical_path' ? 'bg-purple-600' : 'bg-green-500';
  };
              ...
              <>
                  ...
                  <CardContent>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-4 text-sm">
                      <div>
                        ...
                        <p className="font-semibold">{workflow.summary.totalMeals}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Protein Batches</p>
                        <p className="font-semibold">{workflow.summary.totalProteinBatches}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Chiller Cycles</p>
                        <p className="font-semibold">{workflow.summary.blastChillerCycles}</p>
                      </div>
                      <div>
                        <p className="text-muted-foreground">Estimated Idle Time</p>
                        <p className="font-semibold">{formatDuration(workflow.summary.estimatedIdleTime)}</p>
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>Follow these steps in order. Green suggestions show what to do during downtime.</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {workflow.tasks.map((task: any, index: number) => (
                        <div key={task.id} className="relative">
                          ...
                          <div className="flex gap-4">
                            <div className="flex-shrink-0 w-8 h-8 rounded-full bg-purple-600 text-white flex items-center justify-center text-sm font-semibold">
                              {index + 1}
                            </div>
                            <div className="flex-1 pb-4">
                              ...
                              <p className="text-sm text-muted-foreground">
                                {formatDuration(task.startTime)} → {formatDuration(task.endTime)}
                              </p>
                              {task.details && (
                              ...
                              {task.parallelTasks && task.parallelTasks.length > 0 && (
                                <div className="mt-3 p-3 bg-green-50 border-l-4 border-green-500 rounded">
                                  <p className="text-sm font-semibold text-green-700 mb-2">💡 During this time, you can:</p>
                                  <ul className="space-y-1">
                                    {task.parallelTasks.map((suggestion: string, idx: number) => (
